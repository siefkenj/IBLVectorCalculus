...
\subsection*{Lagrange Multipliers}
	...
	\begin{Enum}
		...
		\item What is the relationship between a tangent vector to $C$ at $\vec a$ and
			$\nabla g(\vec a)$?
		\item If $\vec a\in C$ is a maximum, what is the relationship between 
...
\subsection*{The Volume Form}
	...
	add up each pieces contribution with an integral to find the total.  We're continuing the same
	...
	\begin{Enum}
		...
		\item Let $\Delta V(r_0,\theta_0) = r_0\Delta r\Delta\theta$, and let $V(r_0,\theta_0)$ be the exact
			area of the annular (ring-shaped) section you found earlier.  What is $\displaystyle \lim_{\Delta r, \Delta\theta\to 0}
			\frac{V(r_0,\theta_0)}{\Delta V(r_0,\theta_0)}$?  Do we need the $(\Delta r)^2\Delta \theta$ term
			at infinitesimal scales?
	\end{Enum}

	\begin{Def}
		Suppose $\mathcal F$ is a coordinate system for $\R^2$ specified by
		...
		$\d V(a,b)$ obtained by replacing $\Delta w$ with $\d w$ and $(\Delta w)^2$ with $0$ in
	...
	$x-y$ plane is specified in polar coordinates by the first two components
	...
	\begin{Enum}
		...
		\item A discuss is constructed from gluing two cones' bases together.
			...
			cone is $1$.  Describe the surface of the discuss using polar coordinates
