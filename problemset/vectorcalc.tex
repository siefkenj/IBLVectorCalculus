...
\subsection*{Let's get phisicsical}
	...
	and with lower left corner at $\vec 0$.  Assume each face of the cube is
	oriented outwards.
...
\vspace{-1cm}
	\begin{Def}
		Given a volume or area (or line segment) $V$, the \emph{boundary} of $V$
		is denoted $\partial V$ and is assumed to have an outwards orientation.
	\end{Def}
...
\vspace{-.3cm}
	...
	Another amazing coincidence of 3d space is that we can encode all of this information in a single vector.
...
%		\begin{align}
%			R^{\xh}_{\Delta a,\Delta b}(x,y,z) &= \{x\}\times[y,y+\Delta a]\times [z,z+\Delta b],\\
%			R^{\yh}_{\Delta a,\Delta b}(x,y,z) &= [x,x+\Delta a]\times \{y\}\times [z,z+\Delta b], \text{ and }\\
%			R^{\zh}_{\Delta a,\Delta b}(x,y,z) &= [x,x+\Delta a]\times [y,y+\Delta b]\times \{z\}.
%		\end{align}
	\begin{Def}
