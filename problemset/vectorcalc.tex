...
\section*{Surface Integrals}
	...
	\begin{Enum}
		...
		\item Suppose that $r(a,b)$ for $0\leq a\leq 2$ and $-1\leq b\leq 1$ is an isometric
			parameterization of $\mathcal S$ (i.e., $r$ preserves area).  Write down
			an iterated integral to compute $\displaystyle \int_{\mathcal S} f$ with respect
			to surface area.
		\item Suppose that $q(\alpha,\beta)$ for $-3\leq \alpha\leq 0$ and $5\leq \beta\leq 6$
			...
			and $\Delta\beta$ and lower-left corner $(\alpha,\beta)$, then $q(R)$ has surface area
			$Q(\alpha,\beta)\Delta \alpha\Delta\beta$.  Set up an iterated integral to compute
			$\displaystyle \int_{\mathcal S} f$ with respect
			to surface area using $q$ and $Q$.  Justify your intuition with an explanation in
	...
	\sep
	\vspace{-.3cm}
	Suppose $\mathcal P\subset \R^3$ is a plane with parameterization $p(t,s) = t\mat{1\\2\\3} + s\mat{1\\1\\1}$.
	\begin{Enum}
		\item Suppose $R_{(t_0,s_0)}\subset \R^2$ is a rectangle with lower left corner at $(t_0,s_0)$ and 
			sides of length $\Delta t$ and $\Delta s$.  Find a function $Q$ so that $Q(t,s) =\text{area of }p(R_{(t_0,s_0)})$.
			Does $Q$ depend on $t_0$ and $s_0$?  Does this make sense?
		\item Consider the new parameterization $\tilde p$ given by $\tilde p(t,s) = p(t^2,s)$.  What does
